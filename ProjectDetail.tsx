...
import { X, BookOpen, ArrowLeft, Upload, FileText, Check } from 'lucide-react';
import { useState, useRef, useEffect } from 'react';
import { saveImage, getImage, getAllImages, processAndSaveImage } from '../utils/imageStorage';
import { LanguageToggle } from './LanguageToggle';
import { useLanguage } from '../utils/languageContext';

// Import images for Marcel project
const marcelImage1 = image_78cc8121f7f0bfabae7d7e27f086445050a13024;
const marcelImage2 = image_03ab7bda2b8830582eaf387f56a7000a6a8e87d6;
const marcelImage3 = 'https://images.unsplash.com/photo-1634671494478-18ae96453241?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxmYXNoaW9uJTIwYnJhbmRpbmd8ZW58MXx8fHwxNzY4MjI4NDU3fDA&ixlib=rb-4.1.0&q=80&w=1080&utm_source=figma&utm_medium=referral';
const marcelImage4 = 'https://images.unsplash.com/photo-1614036634955-ae5e90f9b9eb?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHx2aXN1YWwlMjBpZGVudGl0eSUyMGRlc2lnbnxlbnwxfHx8fDE3NjgyMjg0NTh8MA&ixlib=rb-4.1.0&q=80&w=1080&utm_source=figma&utm_medium=referral';
const marcelImage5 = 'https://images.unsplash.com/photo-1752650735608-6895f65de119?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxlZGl0b3JpYWwlMjBsYXlvdXR8ZW58MXx8fHwxNzY4MTUwODE3fDA&ixlib=rb-4.1.0&q=80&w=1080&utm_source=figma&utm_medium=referral';
const marcelImage6 = 'https://images.unsplash.com/photo-1739476478863-42b2b97eb647?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHx0eXBvZ3JhcGh5JTIwZGVzaWduJTIwcG9zdGVyfGVufDF8fHx8MTc2ODI0ODI1N3ww&ixlib=rb-4.1.0&q=80&w=1080&utm_source=figma&utm_medium=referral';
const marcelImage7 = 'https://images.unsplash.com/photo-1634406005209-6adbbf0b935f?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxncmFwaGljJTIwZGVzaWduJTIwbWluaW1hbGlzdHxlbnwxfHx8fDE3NjgzMDY3NTF8MA&ixlib=rb-4.1.0&q=80&w=1080&utm_source=figma&utm_medium=referral';

// Graphics gallery images for Marcel project
...
export function ProjectDetail({ projectIndex, onClose }: ProjectDetailProps) {
  ...
  const [uploadedImages] = useState<{[key: string]: string}>({});
  ...
  useEffect(() => {
    ...
    loadImages().catch(error => {
      console.error('Unhandled error in loadImages:', error);
    });
  ...
  useEffect(() => {
    if (akiraHeroImage !== image_672df381cabdb8014754438c02c7657797c99db3) {
      saveImage('akira_hero', akiraHeroImage).catch((error: Error) => {
  ...
  useEffect(() => {
    if (Object.keys(akiraImages).length > 0) {
      Object.entries(akiraImages).forEach(([index, imageData]) => {
        saveImage(`akira_img_${index}`, imageData).catch((error: Error) => {
  ...
  useEffect(() => {
    ...
    if (hasContent) {
      saveImage('marcel_captions', JSON.stringify(marcelCaptions)).catch((error: Error) => {
  ...
  const projects = [
    ...
    },
    {
      ...
      year: '2024',
      category: t('category.womenswear'),
      images: [],
      placeholderImages: [
    ...
    },
    {
      ...
      year: '2023',
      category: t('category.graphicDesign'),
      images: [
        marcelImage1,
        marcelImage2,
        marcelImage3,
    ...
    },
    {
      ...
      year: '2025',
      category: t('category.printDesign'),
      images: []
    },
  ];
  ...
  const handleImageUpload = async (event: React.ChangeEvent<HTMLInputElement>, imageKey: string) => {
    ...
    if (file) {
      try {
        // Compress and save image automatically
        ...
        setProjectImages(prev => ({
          ...prev,
          [imageKey]: compressedImage
        }));
        
        console.log('✅ Image compressed and saved successfully!');
      } catch (error) {
        console.error('Error processing image:', error);
        alert(`Errore nel caricamento: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }
  };

  // Handle image upload for Akira hero image
  const handleAkiraHeroUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    if (file) {
      try {
        const compressedImage = await processAndSaveImage(file, 'akira_hero', {
          maxWidth: 1920,
          maxHeight: 1920,
          quality: 0.85,
          maxSizeMB: 1
        });
        setAkiraHeroImage(compressedImage);
        console.log('✅ Akira hero image compressed and saved!');
      } catch (error) {
        console.error('Error processing Akira hero image:', error);
        alert(`Errore nel caricamento: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }
  };
  ...
  const handleAkiraImageUpload = async (event: React.ChangeEvent<HTMLInputElement>, index: number) => {
    const file = event.target.files?.[0];
    if (file) {
      try {
        const compressedImage = await processAndSaveImage(file, `akira_img_${index}`, {
          maxWidth: 1920,
          maxHeight: 1920,
          quality: 0.85,
          maxSizeMB: 1
        ...
        console.log(`✅ Akira image ${index} compressed and saved!`);
      } catch (error) {
        console.error(`Error processing Akira image ${index}:`, error);
        alert(`Errore nel caricamento: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }
  };
    ...
    <>
      ...
      >
        ...
        >
          ...
          <span className="text-xs uppercase tracking-wider text-neutral-600 font-[Montserrat]">{t('project.back')}</span>
        </motion.button>
        ...
        {/* Language Toggle */}
        <LanguageToggle rightPosition="right-24" />
            ...
            >
              <div className="max-w-6xl mx-auto text-center">
                ...
                <div className="flex items-center justify-center gap-8 mt-6">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="text-xs text-neutral-400 uppercase tracking-wider font-[Montserrat]">Category</div>
                    <div className="text-xs text-neutral-600 font-[Montserrat]">{project.category}</div>
                  </div>
                  <button
            ...
            >
              <div className="max-w-6xl mx-auto text-center">
                ...
                <p className="text-sm leading-relaxed text-neutral-700 font-[Montserrat] mb-8 max-w-2xl mx-auto">
                  {projectIndex === 3
                    ? 'The Akira project, from the Japanese "luminous," "shining," or "brilliant," is rooted in Eastern culture, particularly Japanese culture. The inspirations were manifold, primarily architectural—the shapes of buildings, streets, and rooftops. This gave rise to a veritable stream of consciousness, drawing on various facets of Eastern culture: masks, such as those from Kabuki, the famous Japanese theater, are proof of this, present throughout the collection. The name Akira is attributed to this collection in a somewhat oxymoronic way: in fact, from the very beginning, the color scheme doesn\'t immediately evoke something resplendent, but rather evokes the opposite concept. The collection, in fact, aims to symbolize the beginning of a rebirth, the beginning of a new, seemingly lost brilliance, for which, in the final piece of the collection, we have a concrete metaphor.'
                    : project.description
                  }
                </p>
                ...
                <div className="flex items-center justify-center gap-3 text-xs text-neutral-400 uppercase tracking-wider">
                  ...
                  <span>{t('project.scrollImages')}</span>
                  <div className="w-8 h-[1px] bg-neutral-300"></div>
                </div>
              </div>
            </motion.div>
                      ...
                      >
                        ...
                        <>
                          <input
                            type="file"
                            accept="image/*"
                            onChange={(e) => handleAkiraImageUpload(e, index)}
                            className="hidden"
                            id={`upload-akira-${index}`}
                          />
                          <label
                            htmlFor={`upload-akira-${index}`}
                            className="absolute top-4 left-4 opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer flex items-center gap-1.5 px-3 py-2 bg-neutral-900/80 text-white text-[10px] uppercase tracking-wider hover:bg-neutral-900 rounded-sm backdrop-blur-sm"
                          >
                            <Upload className="w-3 h-3" />
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    {project.category === 'Thesis' && (
                      ...
                      >
                        ...
                        <span>{t('project.viewLookbook')}</span>
                      </button>
                    )}
                    {project.category === t('category.graphicDesign') && (
                      <button
                        onClick={() => setGraphicsGalleryOpen(true)}
                        className="flex items-center gap-1.5 px-3 py-1.5 bg-neutral-200 text-neutral-700 text-[10px] uppercase tracking-wider hover:bg-neutral-300 transition-colors rounded-sm mt-4"
                      >
                        <FileText className="w-3 h-3" />
                        <span>{t('project.viewGraphics')}</span>
                      </button>
                    )}
                  </div>
                </div>
            ...
            <div className="h-full inline-flex" style={{ minWidth: '100%' }}>
              ...
              >
                <div className="max-w-xl">
                  ...
                  {project.category === t('category.menswear') && (
                    <div className="mb-8">
                      <button
                  ...
                  {project.category === t('category.collaboration') && (
                    <div className="mb-8">
                      <button
                ...
                {project.images.length > 0 ? (
                  project.images.map((image, index) => (
                    ...
                    >
                      ...
                      {/* Replace button for all images in all projects */}
                      <input
                        type="file"
                        accept="image/*"
                        onChange={(e) => handleImageUpload(e, `project_${projectIndex}_img_${index}`)}
                        className="hidden"
                        id={`upload-project-${projectIndex}-${index}`}
                      />
                      <label
                        htmlFor={`upload-project-${projectIndex}-${index}`}
                        className="absolute top-4 left-4 opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer flex items-center gap-1.5 px-3 py-2 bg-neutral-900/80 text-white text-[10px] uppercase tracking-wider hover:bg-neutral-900 rounded-sm backdrop-blur-sm"
                      >
                        <Upload className="w-3 h-3" />
                        <span>Replace Image</span>
            ...
            <div className="h-full overflow-y-auto">
              ...
              >
                <div className="max-w-7xl mx-auto text-center">
                  ...
                  <p className="text-sm text-neutral-600 font-[Montserrat] mb-8">
                    Selection of six graphics based on the project "Marcel"
                  </p>
